angular.module("NoContactModal", []).service("modalService", [ "$modal", function($modal) {
    var modalDefaults = {
        backdrop: true,
        keyboard: true,
        modalFade: true,
...
angular.module("formApp.CalcBenefitService", []).service("calcBenefitService", function() {
    this.calculate = function(formData) {
        ...
        if (formData.disabled === true) {
            if (house === 1 && income <= 1915 || house === 2 && income <= 2585 || house === 3 && income <= 3255 || house === 4 && income <= 3925) {
                eligible = true;
            } else if (house >= 5 && income <= (house - 4) * 670 + 3925) {
                eligible = true;
            }
        } else {
            if (house === 1 && income <= 1245 || house === 2 && income <= 1681 || house === 3 && income <= 2116 || house === 4 && income <= 2552) {
                eligible = true;
...
angular.module("formApp.DocumentUploader", []).factory("documentUpload", [ "$http", "$upload", "$q", function($http, $upload, $q) {
    function resizeDocument(file, doc_type, renderFn, callback) {
        ...
        reader.onloadend = function(evt) {
            ...
            tempImg.onload = function() {
                var MAX_WIDTH = 800, MAX_HEIGHT = 600, tempW = tempImg.width, tempH = tempImg.height;
                if (tempW > tempH) {
                    if (tempW > MAX_WIDTH) {
    ...
    return {
        onFileSelect: function($files, $scope, type, user_id) {
            var deferred = $q.defer(), file = $files[0], data = {
                user_id: user_id,
                document_type: type,
                file_name: file.name,
                file_type: file.type
            ...
            if (/(iPad|iPhone|iPod)/g.test(navigator.userAgent)) {
                ...
                resizeDocument(file, type, $scope.renderImg, function(file_url) {
                    ...
                    $scope.upload = $upload.upload({
                        url: "/upload_docs",
                        data: data
                    }).progress(function(evt) {}).success(function(d, status, headers, config) {
                        d["data"] = data;
                        return deferred.resolve(d);
                    }).error(function(err) {
                        return deferred.reject(err);
...
angular.module("formApp.apiFactory", []).factory("API", [ "$http", function($http) {
    return {
        uploadFeedback: function(formData, callback) {
            $http.post("/submit_feedback", JSON.stringify(formData)).success(function(data, status) {
                if (status === 201) {
                    callback(true);
                } else {
                    callback(null);
        ...
        getDocumentStatus: function(user_id, callback) {
            $http.post("/get_doc_status", JSON.stringify({
                user_id: user_id
        ...
        uploadPartialInterviewInfo: function(formData, callback) {
            $http.post("/update_user_info", JSON.stringify(formData)).success(function(data, status) {
                if (status === 201) {
                    callback(true);
                } else {
                    callback(null);
...
angular.module("formApp.documentUploadCtrl", [ "formApp.ngDocumentFullscreen", "formApp.DocumentUploader", "formApp.userDataFactory", "formApp.sampleDocumentsDirective" ]).controller("documentUploadCtrl", [ "$scope", "$upload", "$state", "$stateParams", "$rootScope", "$location", "$window", "documentUpload", "userDataFactory", function($scope, $upload, $state, $stateParams, $rootScope, $location, $window, documentUpload, userDataFactory) {
    ...
    $scope.uploadFile = function($files) {
        ...
        console.log($files);
        documentUpload.onFileSelect($files, $scope, type, $scope.user.user_id).then(function(result) {
...
angular.module("formApp.formController", [ "angularFileUpload", "ui.router", "ui.bootstrap", "ngTouch", "NoContactModal", "formApp.CalcBenefitService", "formApp.infoFooterDirective", "formApp.ngEnterDirective", "formApp.telephoneFilter", "formApp.ssnFilter", "formApp.apiFactory", "formApp.appSubmittedDropdownDirective", "formApp.feedbackFooterDirective", "formApp.modalDirective", "formApp.documentUploadCtrl", "formApp.userDataFactory" ]).controller("formController", [ "$scope", "$state", "$http", "$rootScope", "$upload", "$location", "$cookies", "$window", "API", "userDataFactory", "calcBenefitService", function($scope, $state, $http, $rootScope, $upload, $location, $cookies, $window, API, userDataFactory, calcBenefitService) {
    ...
    $scope.completedName = function() {
        if ($scope.snapForm.$valid && $scope.formData.name.first_name && $scope.formData.name.last_name) {
            ...
            var field_invalid = $scope.snapForm.$error.required, which = "";
    ...
    $scope.completedAddress = function() {
        ...
        } else if ($scope.snapForm.street_address.$valid && $scope.snapForm.zip.$valid && $scope.formData.address.street_address && $scope.formData.address.zip) {
            updateProgress("address");
            $state.go("form.lived-at-duration");
        } else if (($scope.snapForm.street_address.$dirty || $scope.snapForm.zip.$dirty) && ($scope.formData.address.street_address === "" || typeof $scope.formData.address.street_address === "undefined") && ($scope.formData.address.zip === null || typeof $scope.formData.address.zip === "undefined" && $scope.snapForm.street_address.$valid && $scope.snapForm.zip.$valid)) {
            updateProgress("address");
            $scope.has_address = false;
            $state.go("form.lived-at-duration");
        }
    ...
    function calcPotentialImmeditateBenefit() {
        return $scope.formData.total_resources <= 100 || $scope.formData.income <= 150 || $scope.formData.income + $scope.formData.total_resources <= $scope.formData.expenses;
    }
...
angular.module("formApp.interviewCtrl", [ "formApp.userDataFactory", "formApp.apiFactory", "formApp.jSignature", "formApp.CalcBenefitService" ]).controller("interviewCtrl", [ "$scope", "$state", "$rootScope", "$location", "$anchorScroll", "$window", "userDataFactory", "API", "calcBenefitService", function($scope, $state, $rootScope, $location, $anchorScroll, $window, userDataFactory, API, calcBenefitService) {
    ...
    var dd = $scope.today.getDate(), mm = $scope.today.getMonth() + 1, yyyy = $scope.today.getFullYear();
    if (dd < 10) {
        dd = "0" + dd;
    }
    if (mm < 10) {
    ...
    function updateProgressStatus() {
        if ($scope.interviewCompleted.eligibility && $scope.interviewCompleted.household && $scope.interviewCompleted.income && $scope.interviewCompleted.expenses) {
            $scope.interview_progress_status = 2;
        } else if (!$scope.interviewCompleted.eligibility && !$scope.interviewCompleted.household && !$scope.interviewCompleted.income && !$scope.interviewCompleted.expenses) {
            $scope.interview_progress_status = 0;
        } else {
    ...
    function removeHouseholdMembersNotApplying() {
        for (var i = 0; i < $scope.user.household - 1; i++) {
            if (typeof $scope.user.household_members[i] !== "undefined" && ($scope.user.household_members[i].applying == false || typeof $scope.user.household_members[i].name == "undefined")) {
    ...
    $scope.calcPersonsWithIncome = function() {
        ...
        for (var p in $scope.user.household_members) {
            if ($scope.user.household_members.hasOwnProperty(p) && $scope.user.household_members[p].applying === true && typeof $scope.user.household_members[p].income === "number") {
                $scope.personsWithIncome.push({
    ...
    $scope.backToMainSign = function(index) {
        var img_container = document.getElementById("image_wrapper" + index), header = document.getElementById("page_header" + index), pg_header = document.getElementById("sign_header"), tp_sign_page = document.getElementById("topOfSignPage");
...
angular.module("formApp.jSignature", [ "formApp.userDataFactory" ]).directive("jSignature", [ "$timeout", "userDataFactory", function($timeout, userDataFactory) {
    return {
        restrict: "EA",
        link: function(scope, element, attrs, controller) {
            var undoButton = function() {
                var undoButtonStyle = "position:absolute;display:block;margin:0 !important;top:auto";
                var $undoButton = $('<button type="button" class="btn btn-xs btn-default" style="' + undoButtonStyle + '">Undo Last Stroke</button>').appendTo(this.$controlbarLower);
...
angular.module("formApp.formStates", [ "ui.router", "formApp.userDataFactory" ]).config([ "$stateProvider", "$urlRouterProvider", function($stateProvider, $urlRouterProvider) {
...
angular.module("formApp.interviewStates", [ "ui.router", "formApp.userDataFactory" ]).config([ "$stateProvider", "$urlRouterProvider", function($stateProvider, $urlRouterProvider) {
