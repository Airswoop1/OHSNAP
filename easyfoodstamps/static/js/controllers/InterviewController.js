...
angular.module('formApp.interviewCtrl', [
	'formApp.jSignature', 
	...
	$scope.user = User;
	$scope.user.household_members = $scope.user.household_members || [];
	$scope.estimated_benefit = $scope.user.benefit_amount;
	console.log($scope.user);

	$scope.today = new Date();
	$scope.today = $scope.today.toISOString().slice(0, 10).replace(/-/g, '/');
	$scope.user.disabled = 'no';
	$scope.minutes_saved = 0;

	if(!$scope.user.household_members.length) {
		for(var i = $scope.user.household - 1; i; i--) {
			$scope.user.household_members.push({
				applying: false,
		        income: 0,
				show: false,
				relation: 'Select'
	...
	function updateForm() {
		...
		$http.post('/api/pa_snap/' + path, $scope.user)
	...
	$scope.getCity = function() {
		if(typeof $scope.user.address.zip !== 'undefined'){
			getCityFromZip($scope.user.address.zip, function(err, data) {
				if(!err){
	...
	$scope.submitSignedApplication = function() {
		...
		var sig1 = parseB64Data($scope.sig1.data);
		var sig2 = parseB64Data($scope.sig2.data);

		$scope.appSubmissionInProcess = true;
		var user_id = $scope.user.user_id;
		// user_id = '547cc8330640fd2acdebb4f1';
		$http.post('/api/pa_snap/' + user_id + '/', {
			attachments: [{
				doctype: 'SIG1',
				filename: 'sig1.png',
				filetype: sig1.contentType,
				data: sig1.data
			}, {
				doctype: 'SIG2',
				filename: 'sig2.png',
				filetype: sig2.contentType,
				data: sig2.data
			}]
		})
		.success(function(data, status, headers, config) {
